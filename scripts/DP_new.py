...
class DPAgent:
    def __init__(self, gamma: float = 0.99, n_actions: int = 4, n_samples: int = 20, seed: int | None = 0, init: float = 0.0):
        ...
        self.Q_values = np.linspace(self.env.lo, self.env.hi, num=n_samples, dtype=np.float64)
        mesh_q1, mesh_q2 = np.meshgrid(self.Q_values, self.Q_values)
        q_array = np.stack([mesh_q1.ravel(), mesh_q2.ravel()], axis=1).astype(np.float64)
        self.q_array = q_array
        self.K = q_array.shape[0]
    ...
    def _backup(self, i: int, j: int) -> Tuple[float, int]:
        ...
        for a in range(self.n_actions):
            ...
            for k in range(self.K):
                (ni, nj), r = self.env.step_dp((i, j), a , self.q_array[k])
