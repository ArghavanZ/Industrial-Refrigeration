...
class SaveStateHistCallback(BaseCallback):
    """
    Save the (aggregated) state visitation histogram from state_counter wrappers.
    - If freq is None: only save at training end.
    - If freq is an int: also save every `freq` calls to _on_step().
    """
    def __init__(self, save_path: str, freq: int | None = None, verbose: int = 0):
    ...
    def _on_step(self) -> bool:
        ...
        if self.freq is not None and self.n_calls % self.freq == 0:
            self._save_hist(suffix=f"step_{self.n_calls}")
        return True

    def _on_training_end(self) -> None:
        self._save_hist(suffix="final")  # save once at the end

    def _save_hist(self, suffix: str):
        try:
            # One counter per worker process
            counters = self.model.get_env().env_method("get_counter")
        except Exception as e:
            if self.verbose:
                print(f"[SaveStateHistCallback] env_method error: {e}")
            return

        counters = [c for c in counters if c is not None]
        if not counters:
            if self.verbose:
                print("[SaveStateHistCallback] No counters returned; is state_counter applied in make_env?")
            return

        total = np.sum(counters, axis=0).astype(np.int64)
        base, ext = os.path.splitext(self.save_path)
        path = f"{base}_{suffix}{ext}" if suffix else self.save_path
        os.makedirs(os.path.dirname(path), exist_ok=True)
        np.savez_compressed(path, hist=total)
        if self.verbose:
